...
static struct TLNode* tl_list_create_node(TLMemoryArena *arena, void *value) {
    BKS_STACK_PUSHA("0x%p, 0X%p", arena, value)
...
u64 tl_list_length(TLList* list) {
    BKS_STACK_PUSHA("0x%p", list)
    BKS_STACK_POPV(list->length)
}

void* tl_list_search(TLList* list, b8 (*PFN_filter)(void *value)) {
    BKS_STACK_PUSHA("0x%p, 0x%p", list, PFN_filter)
    if (list == NULL) {
...
void tl_list_foreach(TLList* list, void (*PFN_handler)(void *value)) {
    BKS_STACK_PUSHA("0x%p, 0x%p", list, PFN_handler)
    if (list == NULL) {
        BKSERROR("TLList is NULL")
        BKS_STACK_POP
    }
...
void tl_list_add(TLList* list, void *value) {
    BKS_STACK_PUSHA("0x%p, 0x%p", list, value)
    if (list == NULL) {
        BKSERROR("TLList is NULL")
        BKS_STACK_POP
    }
...
b8 tl_list_after(TLList* list, void *item, void *value) {
    BKS_STACK_PUSHA("0x%p, 0x%p, 0x%p", list, item, value)
    if (list == NULL) {
        BKSERROR("TLList is NULL")
...
void* BKS_STACK_pop(TLStack* stack) {
    BKS_STACK_PUSHA("0x%p", stack)
    void* value = BKS_STACK_peek(stack);
...
u64 BKS_STACK_length(TLStack* stack) {
    BKS_STACK_PUSHA("0x%p", stack)
    BKS_STACK_POPV(tl_list_length(stack))
}
...
void tl_map_put(TLMap* map, const char *key, void *value) {
    BKS_STACK_PUSHA("0x%p, %s, 0x%p", map, key, value)
...
u16 tl_map_length(TLMap* map) {
    BKS_STACK_PUSHA("0x%p", map)
    const u16 length = map->length;
    BKS_STACK_POPV(length)
}
...
TLIterator* tl_map_keys(TLMap* map) {
    ...
    if (map->length == 0) BKS_STACK_POPV(NULL)
