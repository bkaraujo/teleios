...
void tl_map_destroy(TLMap* map) {
    ...
    if (map == NULL) {
        TLWARN("Attempted to destroy a NULL TLMap")
        TL_PROFILER_POP
    }
...
TLList* tl_map_get(TLMap* map, const TLString* key) {
    ...
    if (map == NULL) {
        TLWARN("Attempted to get from a NULL TLMap")
        TL_PROFILER_POP_WITH(NULL)
    }
...
TLList* tl_map_get_or_create(TLMap* map, const TLString* key) {
    ...
    if (map == NULL) {
        TLWARN("Attempted to get_or_create from a NULL TLMap")
        TL_PROFILER_POP_WITH(NULL)
    }
...
void tl_map_put(TLMap* map, TLString* key, void* value) {
    ...
    if (map == NULL) {
        TLWARN("Attempted to put into a NULL TLMap")
        TL_PROFILER_POP
    }
...
b8 tl_map_contains(TLMap* map, const TLString* key) {
    ...
    if (map == NULL) {
        TLWARN("Attempted to read a NULL TLMap")
...
TLList* tl_map_remove(TLMap* map, const TLString* key) {
    ...
    if (map == NULL) {
        TLWARN("Attempted to remove a NULL TLMap")
        TL_PROFILER_POP_WITH(NULL)
    }
...
void tl_map_clear(TLMap* map) {
    ...
    if (map == NULL) {
        TLWARN("Attempted to clear a NULL TLMap")
        TL_PROFILER_POP
    }
