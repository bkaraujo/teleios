...
TLScene* tl_config_get_scene(const TLString* name) {
    ...
    // ==========================================
    if (name == NULL) {
        TLERROR("Attempted to get scene with NULL name");
        TL_PROFILER_POP_WITH(NULL)
    }

    TLScene* scene = tl_memory_alloc(
        global->allocator,
        TL_MEMORY_SCENE,
        sizeof(TLScene)
    );
    ...
    // ==========================================
    TLAllocator* allocator = tl_memory_allocator_create(
        TL_KIBI_BYTES(4),
        TL_ALLOCATOR_LINEAR
    );

    // Resolves to application.scene.#
    const TLString* path_prefix = NULL;

    for (scene->index = 0; scene->index <= U8_MAX; ++scene->index) {
        ...
        // ==========================================
        {
            ...
            const TLString* index_str = tl_number_u32_to_char(allocator, scene->index, 10);
            tl_string_builder_append(builder, index_str);

            path_prefix = tl_string_builder_build(builder);
    ...
    TLDEBUG("Found scene '%s' at index %d", tl_string_cstr(name), scene->index);
    ...
    // ==========================================
    const TLString* load_path = tl_config_read_script_path(allocator, path_prefix, "load");
    if (load_path != NULL) {
    ...
    const TLString* unload_path = tl_config_read_script_path(allocator, path_prefix, "unload");
    if (unload_path != NULL) {
    ...
    const TLString* frame_begin_path = tl_config_read_script_path(allocator, path_prefix, "frame_begin");
    ...
    const TLString* frame_end_path = tl_config_read_script_path(allocator, path_prefix, "frame_end");
